(function (window, undefined) {
    ...
    $(document).ready(function () {
        ...
        if (spHostUrl && currentAuthority) {
            ...
            appendSPHostUrlToForms(spHostUrl, currentAuthority);
        }
    });
    ...
    // Appends SPHostUrl as query string to all the links which point to current domain.
    function appendSPHostUrlToForms(spHostUrl, currentAuthority) {
        $("form")
            .filter(function () {
            ...
            .each(function () {
                var pieces = this.action.split('?', 2);
                var search = "";
                if (pieces.length > 1)
                    search = pieces[1];
                if (!getSPHostUrlFromQueryString(search)) {
                    if (search.length > 0) {
                        search += "&" + SPHostUrlKey + "=" + spHostUrl;
                    }
                    else {
                        search = "?" + SPHostUrlKey + "=" + spHostUrl;
                    }
                }
                this.action = pieces[0] + search;
            });
    }
